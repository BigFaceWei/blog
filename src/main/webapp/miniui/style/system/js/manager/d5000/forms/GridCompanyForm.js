...
Ext.GridCompanyForm = Ext.extend(Ext.form.FormPanel, {
	...
	initComponent: function() {
		...
		var companyGradeStore = new Ext.data.JsonStore({
	    	url: contextpath + '/tbp/sysmanager/areano.do?method=listDic&type=companyGrade',
	    	fields: ['DIC_ID', 'DIC_NAME']
	    });
		companyGradeStore.load();
		...
		this.items = [{
			...
			items: [{
				...
				items: [{
					...
					name: 'CONTACTNUMBER',
					xtype:'textfield',
					regex: /(^[1][3-8]\d{9})$|^((\d{7,8})|(\d{3,4})-(\d{7,8})|(\d{3,4})-(\d{7,8})-(\d{1,4})|(\d{7,8})-(\d{1,4}))$/,
				...
				columnWidth: .5,
					...
					items: [{
						...
						fieldLabel: '公司级别',
						hiddenName: 'COMPANYGRADE',
						xtype: 'combo',
						displayField: 'DIC_NAME',
						valueField: 'DIC_ID',
						typeAhead: true,
				        triggerAction: 'all',
				        store: companyGradeStore,
				        emptyText:'请选择公司级别...',//默认值  
			            blankText: '公司级别为必填项',
			            editable: false,//不允许输入  
			            allowBlank: false,
				        forceSelection: true,
		...
		Ext.GridCompanyForm.superclass.initComponent.call(this);
