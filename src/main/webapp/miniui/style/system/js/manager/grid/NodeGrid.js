Ext.NodeGrid = Ext.extend(Ext.grid.EditorGridPanel, {
	initComponent: function() {
		...
		Ext.NodeGrid.superclass.initComponent.call(this);
	...
	sort: function () {
        ...
        for (var i = 0; i < ds.getCount(); i++) {
            sortIndex.push(ds.data.items[i].data.TYPEID);
        }
        Ext.Ajax.request({
            url: contextpath + '/tbp/sysmanager/tree.do?method=sort&sortTable=nodeType',
            params: {sortArray: Ext.encode(sortIndex)},
            success: function(response, op){
                if(response.responseText == '1') {
                	//Ext.Msg.alert('系统信息', '新的节点类型顺序已保存！');
	...
	editNode: function(){
        ...
        for ( var i = 0, len = records.length; i < len; i++) {
            ...
            Ext.Ajax.request({
	            url: contextpath + '/tbp/sysmanager/areano.do?method=saveVirtualNode',
	...
	deleteNode: function () {
	    ...
	    Ext.Ajax.request({
			url: contextpath + '/tbp/sysmanager/areano.do?method=deleteVirtualNode&typeId='+rec.get("TYPEID"),
			success: function (response, opts) {
