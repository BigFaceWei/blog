Ext.WDcTab = function() {
	...
	Ext.WDcTab.superclass.constructor.call(this, {
		activeTab: 0,
		plain: true,
		frame: true,
		tbar: [{
		...
		items: [_this.getDcBaseForm(),_this.getWDcForm()]
    });
	
};

Ext.extend(Ext.WDcTab, Ext.TabPanel, {
	
	init: function(stationId, stationType) {
		this.getDcBaseForm().loadDc(stationId);
		this.getWDcForm().loadWDc(stationId, stationType);
		// 操作标志，1表示新增、2表示修改
		this.operFlag = 2;
	},
	...
	savaStation: function() {
		var _this = this;
		if (this.getDcBaseForm().getForm().isValid() && this.getWDcForm().getForm().isValid()) {
			this.setActiveTab(1);
			this.setActiveTab(0);
			Ext.Ajax.request({
				...
				params: {
					...
					dcExtendInfo: Ext.encode(this.getWDcForm().getForm().getValues())
				},
