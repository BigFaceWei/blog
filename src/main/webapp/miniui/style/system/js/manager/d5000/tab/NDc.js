Ext.NDcTab = function() {
	...
	Ext.NDcTab.superclass.constructor.call(this, {
		activeTab: 0,
		plain: true,
		frame: true,
		tbar: [{
		...
		items: [_this.getDcBaseForm(),_this.getNDcForm()]
    });
	
};

Ext.extend(Ext.NDcTab, Ext.TabPanel, {
	
	init: function(dcId, stationType) {
		this.getDcBaseForm().loadDc(dcId);
		this.getNDcForm().loadNDc(dcId, stationType);
		// 操作标志，1表示新增、2表示修改
		this.operFlag = 2;
	},
	...
	savaStation: function() {
		var _this = this;
		if (this.getDcBaseForm().getForm().isValid() && this.getNDcForm().getForm().isValid()) {
			this.setActiveTab(1);
			this.setActiveTab(0);
			Ext.Ajax.request({
				...
				params: {
					...
					dcExtendInfo: Ext.encode(this.getNDcForm().getForm().getValues())
				},
