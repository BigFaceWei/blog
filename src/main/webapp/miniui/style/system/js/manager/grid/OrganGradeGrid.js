Ext.OrganGradeGrid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	initComponent: function() {
		...
		this.columns = [
		    ...
		    {header:'机构级别', width:150, dataIndex:'GRADENAME', sortable: true, editor: {
		    	xtype: 'textfield'
		    }}
		   ];
		
		Ext.OrganGradeGrid.superclass.initComponent.call(this);
	...
	sort: function () {
        ...
        for (var i = 0; i < ds.getCount(); i++) {
            sortIndex.push(ds.data.items[i].data.GRADEID);
        }
        Ext.Ajax.request({
            url: contextpath + '/tbp/sysmanager/tree.do?method=sort&sortTable=organGrade',
            params: {sortArray: Ext.encode(sortIndex)},
            success: function(response, op){
                if(response.responseText == '1') {
                	//Ext.Msg.alert('系统信息', '新的机构级别顺序已保存！');
	...
	addNode: function() {
        ...
        var p = new Plant({ GRADEID:'', GRADENAME:'' });
	...
	editNode: function(){
        ...
        for ( var i = 0, len = records.length; i < len; i++) {
            ...
            if(record.get('GRADENAME') == '') {
            	this.startEditing(store.indexOf(record), 1);
            	return;
            }
            
            Ext.Ajax.request({
	            url: contextpath + '/tbp/sysmanager/areano.do?method=saveOrgangrade',
	...
	deleteNode: function () {
	    ...
	    Ext.Ajax.request({
			url: contextpath + '/tbp/sysmanager/areano.do?method=deleteOrgangrade&gradeId='+rec.get("GRADEID"),
			success: function (response, opts) {
