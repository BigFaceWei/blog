function dateset(rootUrl,name,type) {
	...
	var pars= "dialogWidth:197px;dialogHeight:220px;dialogLeft:"+showx+"px;dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "

	var retval=window.showModalDialog(rootUrl + "/tbp/page/control/Calendar.htm",window,pars);
	...
	if(type == 1) {
		...
		if(retval!=null)
		{
		  document.getElementById(name).value=retval.substr(0,10);	  
		}
	}
}
...
{   var event = window.event || arguments.callee.caller.arguments[0];
    ...
    if (event.srcElement.tagName == "TD") 
        ...
        { 
            sDate = document.getElementById("Year").value + "-" + document.getElementById("Month").value + "-" + $(event.srcElement).text();
