...
(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            ...
            showQueryForm: false,      //显示查询表单
            showQueryFormToggleBtn: false, //显示切换查询表单按钮
            autoload: true, //自动加载
        ...
        var g = {
            ...
            changeSort: function(th) { //change sortorder
                ...
                if (p.sortname == 'noSort')
                	return;
                
                if (typeof(p.sortorder)=='undefined' || p.sortorder == '')
                	p.sortorder = 'asc';
            ...
            cellProp: function(n, ptr, pth) {
                ...
                $(this).dblclick(function() {
                	var ids = [];
                	$("td", $(this).parent()[0]).each(function() {
                        ids.push(this.innerText);
                    });
                	
                	if (p.onRowDbClicked) {
                		p.onRowDbClicked(this, ids);
                	}
                });
...
//                });
                    ...
                    function() {
                        $("input.itemchk", this).each(function() {
                            ...
                            $(this).click(function() {
                                ...
                                var ids = [];
                            	$("td", $(this).parent().parent().parent()).each(function() {
                                    ids.push(this.innerText);
                                });
                                if (p.onRowChecked) {
                                	p.onRowChecked(this, ids);
                                }
                            });
                        });
                        $gF.call(this);
        ...
        //setup thead	
        var columnCount = $('thead tr:first th', g.hDiv).length;
        var operationColumnCount = 0;
        var operationColumnWidth = 0;
        for(var i = 0; i<p.colModel.length; i++) {
        	if(p.colModel[i].name=='_operation') {
        		operationColumnCount++;
        		operationColumnWidth = operationColumnWidth + parseInt(p.colModel[i].width);
        	}
        }
        var clientWidth = g.gDiv.scrollWidth - 80 - operationColumnWidth;
        columnCount = columnCount - operationColumnCount;
			...
			(
			 	function() {
			 	    ...
			 	    if (this.width.length == 0) {
			 	       this.width = Math.ceil(clientWidth/columnCount);
			 	    } else if (this.width.substring(this.width.length - 1) == '%') {
			 	       this.width = this.width.substr(0, this.width.length - 1);
			 	       var tempWidth = parseInt(this.width);
			 	       this.width = Math.ceil(clientWidth*this.width/100);
			 	    } 
	                //////////////////////////////////////////////////////////////////
			 	    if (this.abbr != 'operation')
			 	    	$(thdiv).css({ textAlign: this.align, width: this.width + 'px'});
			 	    else
			 	    	$(thdiv).css({ textAlign: this.align, width: '50px'});
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = '<a id="blankHref"/><div class="pGroup"><div class="pFirst pButton" title="转到第一页"><span></span></div><div class="pPrev pButton" title="转到上一页"><span></span></div> </div><div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">当前 <input type="text" size="4" value="1" /> ,总页数 <span> 1 </span></span></div><div class="btnseparator"></div><div class="pGroup"> <div class="pNext pButton" title="转到下一页"><span></span></div><div class="pLast pButton" title="转到最后一页"><span></span></div></div><div class="btnseparator"></div><div class="pGroup"> <div class="pReload pButton" title="刷新"><span></span></div> </div> <div class="btnseparator"></div><div class="pGroup"><span class="pPageStat"></span></div>';
            $('div', g.pDiv).html(html);

            $('.pReload', g.pDiv).click(function() { 
                if (p.onChangePage)
                    p.onChangePage(p.page);
                else
                    this.populate();
            });
            ...
            if (p.useRp) {
                ...
                for (var nx = 0; nx < p.rpOptions.length; nx++) {                	
                    if (p.rp == p.rpOptions[nx]) {check++;sel = 'selected="selected"';} else sel = '';
                    if (check==0&&p.rp<p.rpOptions[nx]){check++;sel = 'selected="selected"';}
        ...
        if (p.title) {
            ...
            //  add by yanfei
            if (p.showQueryFormToggleBtn) {
                var vsble = (p.showQueryForm)?'vsble' : '';
                $(g.mDiv).append('<div class="queryformtitle ' + vsble +'" title="显示隐藏查询表单"><span></span></div>');
                $('div.queryformtitle', g.mDiv).click
                    (
                        function() {
                            p.toogleQueryForm();
                            $(this).toggleClass('vsble');
        ...
        if ($('th', g.hDiv).length) {
			...
			(
			 	function() {
			 	    ...
			 	    if  ($("[name=tv_checkbox_all]").length >0){
			 	    	
				 	    $('tbody', g.nDiv).append('<tr><td class="ndcol1"><input type="checkbox" ' + chk + ' class="togCol noborder" value="' + (parseInt(cn)+1).toString() + '" /></td><td class="ndcol2">' + this.innerHTML + '</td></tr>');			 		   
			 	      }else{
