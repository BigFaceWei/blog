...
scrollDoor.prototype = {  
	...
	sd : function(menus,divs,openClass,closeClass){  
		 ...
		 for(var i = 0 ; i < menus.length ; i++){   
			  ...
			  _this.$(menus[i]).ondblclick=function(){
			  	   _this.resetMenus(this.value);
			  }
		 }  
	 },  
	$ : function(oid){  
	...
	resetMenus:function(menuid){
		try{
					var _this = this; 
					if(typeof(_dbclickScrollDoorMenus)!='undefined'){
				   		_dbclickScrollDoorMenus(_this.$(_this.menus[menuid]));
				   }
		}catch(e){
			alert(e.message);
		}
